...
def im_detect_bbox_aug(cfg, model, images, device):
    ...
    boxlists_i = im_detect_bbox(cfg, model, images, cfg.INPUT.MIN_SIZE_TEST,
    ...
    if cfg.TEST.BBOX_AUG.H_FLIP:
        boxlists_hf = im_detect_bbox_hflip(
            cfg, model, images, cfg.INPUT.MIN_SIZE_TEST,
            cfg.INPUT.MAX_SIZE_TEST, device)
    ...
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        ...
        boxlists_scl = im_detect_bbox_scale(
            cfg, model, images, scale, max_size, device)
        add_preds_t(boxlists_scl)

        if cfg.TEST.BBOX_AUG.SCALE_H_FLIP:
            boxlists_scl_hf = im_detect_bbox_scale(
                cfg, model, images, scale, max_size, device, hflip=True)
    ...
    for boxlist in boxlists:
        results.append(filter_results(
            boxlist, cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES,
            cfg.MODEL.ROI_HEADS.SCORE_THRESH, cfg.MODEL.ROI_HEADS.NMS,
            cfg.MODEL.ROI_HEADS.DETECTIONS_PER_IMG))

    return results


def im_detect_bbox(cfg, model, images, target_scale, target_max_size, device):
    """
...
def im_detect_bbox_hflip(
        cfg, model, images, target_scale, target_max_size, device):
    """
...
def im_detect_bbox_scale(
        cfg, model, images, target_scale, target_max_size, device, hflip=False):
    """
