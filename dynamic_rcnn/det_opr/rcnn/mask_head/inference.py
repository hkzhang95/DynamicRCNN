...
def paste_mask_in_image(im_mask, mask, box, im_h, im_w, thresh=0.5):
    TO_REMOVE = 1
...
class Masker(object):
    ...
    def forward_single_image(self, masks, boxes):
        ...
        masks, scale = expand_masks(masks, padding=self.padding)
        boxes = expand_boxes(boxes.bbox, scale).to(dtype=torch.int32)

        if len(masks) > 0:
            res = torch.zeros(
                (len(masks), 1, im_h, im_w), dtype=torch.uint8,
                device=masks.device)
            for idx, (m, b) in enumerate(zip(masks, boxes)):
                paste_mask_in_image(
                    res[idx, 0, :, :], m[0], b, im_h, im_w, self.threshold)
        else:
